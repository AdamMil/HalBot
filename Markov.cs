...
{ public Node(string text) { Text=text; }
  ...
  public int CompareTo(object obj) { return string.Compare(Text, ((Node)obj).Text); }
  ...
  { public int Compare(object a, object b)
    { Node na=a as Node, nb=b as Node;
...
{ static Brain()
  ...
  { if(fore.Count==0) return "Sorry, I have no brain.";
    ...
    { DictionaryEntry de = des[rand.Next(des.Length)];
      ...
      if(keyword!=null)
      { bnode=(Node)back[keyword]; fnode=(Node)fore[keyword];
  ...
  { string[] bits = Tokenize(input, false);
    ...
    { resp[i] = GetResponse(bits);
      ...
      if(resp[i].Text.Length>300 || resp[i].Text==lastReply) resp[i].Probability /= 3;
    }
  ...
  void AddWords(ArrayList list, Hashtable nodes, Node node, ref float probability)
  { while(true)
    { Node child = node.RandomNode();
      if(child==null)
      { int k = list.Count-WindowSize+1;
        node = (Node)nodes[(string)list[k]];
        while(node!=null && ++k<list.Count) node = node.GetNode((string)list[k]);
        if(node!=null) child = node.RandomNode();
        if(child==null) break;
      }
      if(child.Text==null) break;
      list.Add(child.Text);
      probability += (float)child.Count/node.Total;
      node = child;
    }
  }
  ...
  static Node AddNode(Hashtable nodes, string text)
  ...
  { Node n = AddNode(nodes, chain[index]);
